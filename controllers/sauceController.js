...
exports.canUserSubmit = canUserSubmit = async (req, res, next) => {
  try {
    ...
    // If we are end of stack, go to client
    if (isLastMiddlewareInStack || !IsEmailVerified) {
      //return to client
      return res.status(200).send({
        isGood: IsEmailVerified, //user can/cannot update
        msg:
          "You are ineligible to submit a sauce at this time. Please verify email first."
      });
    } else {
      // Go to next middleware
      res.locals.canUserSubmit = IsEmailVerified;
      return next();
    }
  } catch (err) {
    console.log(err);
